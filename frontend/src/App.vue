...
<template>
  <header class="app-header">
    ...
    <h1 class="app-title">Sistema de Matrículas</h1>
...
<style>
/* Reset básico e estilo do fundo (inalterado) */
body {
  margin: 0;
  padding: 0;
...
/* ESTILOS PARA O CABEÇALHO DA APLICAÇÃO (A MÁGICA ESTÁ AQUI) */
.app-header {
  /* A div do header agora só precisa garantir o espaçamento */
  margin-bottom: 50px;
}
.header-icon-wrapper {
  position: absolute; /* Posiciona o ícone de forma flutuante */
  top: -25px;      /* << Move o ícone para CIMA do texto */
  left: 50%;       /* << Move o ícone para o MEIO da letra 'S' */
  transform: translateX(-50%); /* << Ajuste fino para centralizar o ícone perfeitamente */
  z-index: 10;
}
...
.app-title {
  ...
  position: relative; /* << IMPORTANTE: Transforma o H1 na área de referência */
  display: inline-block; /* Ajusta o tamanho do H1 ao texto */
  padding: 0 10px;
}
.first-letter {
  position: relative; /* Define a letra 'S' como um ponto de referência */
}
</style>